...
export function Destination(){
    ...
    return(
        <div>
            {window.innerWidth < 1024 && <p className="font-barlow text-base text-white tracking-[0.16875em] text-center xs:text-left xs:mx-9 xs:mt-10 xs:text-xl xl:ml-[10.4rem] lg:text-[1.75rem]"><span className="text-gray font-bold mr-4">01</span>PICK YOUR DESTINATION</p>}
            {data.destinations.filter(planets => planets.name == planet).map((filteredPlanet)=>{
                    return(
                        <div key={filteredPlanet.name} className="flex flex-col items-center lg:flex-row  lg:justify-evenly xl:justify-center xl:gap-40">
                            ...
                            {window.innerWidth >= 1024 && <p className="font-barlow text-base text-white tracking-[0.16875em] text-center xs:text-left xs:mx-9 xs:mt-10 xs:text-xl lg:mx-0 lg:text-[1.75rem] lg:relative lg:bottom-44 xl:bottom-20"><span className="text-gray font-bold mr-4">01</span>PICK YOUR DESTINATION</p>}
                            <img src={`${filteredPlanet.images.png}`} alt="planet-image" className="w-[10.63rem] h-[10.63rem] xl:w-[27.813rem] xl:h-[27.813rem] my-7 xl:mt-0 lg:-mt-20 float xs:w-[18.75rem] xs:h-[18.75rem] xs:my-12 mx-auto lg:mx-0"/>
                            </div>
                            <div className="lg:mt-12">
                                ...
                                <p className="text-[#D0D6F9] leading-6 xs:leading-7 lg:leading-8 lg:w-[27.75rem] xs:w-[34rem] font-reg text-center text-[0.94rem] mx-6 lg:mx-0 xs:text-base lg:text-lg lg:text-left">{filteredPlanet.description}</p>
